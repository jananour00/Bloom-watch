export function normalizeNDVI(ndvi) {
  // Normalize NDVI from -1 to 1 range to 0 to 1
  return (ndvi + 1) / 2
}

export function calculateRollingAverage(values, windowSize = 3) {
  const result = []
...
export function processBloomData(rawData) {
  // Sort by date
...
export function filterByBloomStage(data, stages) {
  return data.filter((item) => stages.includes(item.bloomStage))
}

export function calculateBloomStatistics(data) {
